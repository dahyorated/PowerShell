<#
 ...
 .DESCRIPTION
   ...
   It isn't suitable for the huge objects produced by some of the cmdlets such as Get-Process, but fine for simple objects.
 .EXAMPLE
   $array=@()
   $array += Get-Process wi* | Select-Object Handles,NPM,PM,WS,VM,CPU,Id,ProcessName 
   ConvertTo-YAML $array
 .PARAMETER Object 
   The object that you want converted to YAML.
 .PARAMETER Depth
   The depth that you want your YAML to contain.
 .PARAMETER Nesting Level
   This is for internal use only. It is required for formatting.
#>
Function ConvertTo-YAML
{
    ...
    PROCESS
    {
        If ($null -eq $inputObject -and !($null -ne $inputObject)) { $p += 'null'; return $p } # if it is null return null
        ...
        try
        {
                    ...
                    'SqlDataReader', 'datarow', 'ScriptBlock', 'type') -notcontains $type)
            {
                ...
                elseif ($null -ne ($inputObject | gm -membertype Properties | Select-Object name | Where-Object name -like 'Keys')) { $Type = 'generic' } #use dot notation
            ...
            switch ($Type)
            {
                ...
                'DateTime'
                    { $inputObject.ToString('s') } # s=SortableDateTimePattern (based on ISO 8601) using local time
                'Byte[]'
                    {
                ...
                'Array'
                    { "$($inputObject | ForEach-Object { "`r`n$padding- $(ConvertTo-YAML -inputObject $_ -depth $depth -NestingLevel ($NestingLevel + 1))" })" }
                'HashTable'
                    {
                ...
                'PSObject'
                    { ("$($inputObject.PSObject.Properties | ForEach-Object { "`r`n$padding $($_.Name): " + (ConvertTo-YAML -inputObject $_ -depth $depth -NestingLevel ($NestingLevel + 1)) })") }
                'generic'
                    { "$($inputObject.Keys | ForEach-Object { "`r`n$padding  $($_):  $(ConvertTo-YAML -inputObject $inputObject.$_ -depth $depth -NestingLevel ($NestingLevel + 1))" })" }
                'Object'
                    { ("$($inputObject | Get-Member -membertype properties | Select-Object name | ForEach { "`r`n$padding $($_.name):   $(ConvertTo-YAML -inputObject $inputObject.$($_.name) -depth $NestingLevel -NestingLevel ($NestingLevel + 1))" })") }
                'XML'
                    { ("$($inputObject | Get-Member -membertype properties | where-object { @('xml', 'schema') -notcontains $_.name } | Select-Object name | ForEach { "`r`n$padding $($_.name):   $(ConvertTo-YAML -inputObject $inputObject.$($_.name) -depth $depth -NestingLevel ($NestingLevel + 1))" })") }
                'DataRow'
                    { ("$($inputObject | Get-Member -membertype properties | Select-Object name | ForEach { "`r`n$padding $($_.name):  $(ConvertTo-YAML -inputObject $inputObject.$($_.name) -depth $depth -NestingLevel ($NestingLevel + 1))" })") }
