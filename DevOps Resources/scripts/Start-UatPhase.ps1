<#
.SYNOPSIS
Update all requested release definitions to start with UAT-EUW instead of QAT-DEV.

.DESCRIPTION
This updates all requested release definitions found in -QueryPath to start with UAT-EUW instead of QAT-DEV.
	...
	if ($relDefs.count -gt 0)
	{
		...
		$relDefs |
			...
			ForEach-Object {
				...
				$isChanged = $false;
				# Reset variables to standard values
				$varName = 'system.debug';
				$varStandardValue = 'false';
				if ($null -ne $relDef.variables.$varName)
				{
					# set to false if needed
					$varValue = $relDef.variables.$varName.value;
					if ($varValue -ne $varStandardValue)
					{
						$isChanged = $true;
						$relDef.variables.$varName = $varStandardValue;
					}
				}
				...
				foreach ($relEnv in $relEnvs)
				{
					...
					if ($stagename -eq 'QAT-EUW' -and $relEnv.conditions.Count -ne 0)
					{
						$isChanged = $true;
						$relEnv.conditions = @();
					};
